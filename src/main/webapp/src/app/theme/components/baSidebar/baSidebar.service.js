(function() {
  ...
  function baSidebarServiceProvider() {
    ...
    /** @ngInject */
    this.$get = function($state, layoutSizes, userService) {
      return new _factory();
      ...
      function _factory() {
        ...
        this.getMenuItems = function() {
          ...
          var menuItems = states.filter(function(item) {
            var stateName = item.name.startsWith('app.') ? item.name.split('app.')[1] : item.name;
            return item.topLevel && (item.protected ? userService.checkPermission(stateName) : true);
          });
          ...
          return $state.get()
              ...
              .map(function(s) {
                ...
                return {
                  name: s.name,
                  title: s.title,
                  topLevel: (s.name.match(/\./g) || []).length == 1,
